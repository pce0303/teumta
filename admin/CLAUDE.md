...
(2026-08-07 갱신) Tag API 구현됨 — `GET /api/tags`(placeCount 포함),
`POST /api/admin/tags`, `DELETE /api/admin/tags/:id`.
장소 폼에서 태그 선택/생성/삭제 전부 지원한다.

새 태그 기능을 추가할 때도 가짜 태그/임의 API 가정 금지 원칙은 유지한다.

---

# 10. Admin Authentication

(2026-08-07 갱신) 최소 관리자 인증 구현·배포됨:

```text
POST /api/admin/login (ADMIN_PASSWORD 검증, 12시간 HMAC 토큰)
/api/admin/* 전체 Bearer 보호 (미설정 시 fail closed 503)
로그인 rate limit (IP별 15분 창 실패 10회 → 429)
```

유지해야 하는 원칙:

* 클라이언트 번들에 비밀번호나 secret을 하드코딩하지 않는다 (토큰만 localStorage 저장).
* `ADMIN_PASSWORD`는 서버 환경변수에만 존재한다. 변경 시 기존 토큰 전부 무효화(의도된 설계).
* 임의로 복잡한 사용자/회원 시스템을 추가하지 않는다.

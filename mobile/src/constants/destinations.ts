 ...
 * 홈 화면에서 소개하는 대표 목적지.
 *
 * 선정 기준(전부 실제 API 응답으로 확인):
 *  1. 집중률 예측 제공 — 여행 전 "언제 덜 붐비나" 판단 근거
 *  2. 우회 코스가 실제로 생성 — 걸어서 갈 로컬 장소 필요
 *     (만장굴처럼 주변이 비면 코스 0개라 제외)
 *  3. 실제로 붐비는 대표 관광지 — 오버투어리즘 완화의 출발점
 *  4. 지역당 2곳
 *
 * 실시간 혼잡도(SK)는 기준에서 제외 — 커버리지가 좁아(경복궁·해운대·전주한옥마을 등)
 * 요구하면 후보가 거의 안 남는다. 혼잡도 없는 곳은 상세에서 집중률 예측으로 대체.
 *
